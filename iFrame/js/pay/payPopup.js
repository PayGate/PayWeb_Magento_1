...
function createPayOrder(baseurl) {
    ...
    jQuery.ajax({
        ...
        jQuery("#payPopupContent").append("<form target='myIframe' name='paygate_checkout' id='paygate_checkout' action='https://secure.paygate.co.za/payweb3/process.trans' method='post'><input type='hidden' name='PAY_REQUEST_ID' value='" + params['PAY_REQUEST_ID'] + "' size='200'><input type='hidden' name='CHECKSUM' value='" + params['CHECKSUM'] + "' size='200'></form><iframe id='payPopupFrame' name='myIframe' src='#' style='min-width: 995px;'></iframe><script type='text/javascript'>document.getElementById('paygate_checkout').submit();</script>");
    });
}